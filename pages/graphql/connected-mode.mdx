...
In this guide, we will learn how to build a Sitecore Next.js component using GraphQL in connected mode. We will build a main navigation so that it pulls data from Sitecore. In contrast
to the [integrated mode](/graphql/integrated-mode), we will *only* build the Next.js component and *not* the corresponding Sitecore rendering. This mode is exactly the same as using a GraphQL API in any JavaScript application.

The use case here is sometimes there are components that should *always* appear at a certain spot and we do *not* want to allow content editors the ability to move them to different placeholders. 
...
```
1. publish templates
1. `src/rendering` run `jss graphql:update`
src\rendering\src\temp\GraphQLIntrospectionResult.json
1. npm run bootstrap or restart rendering
