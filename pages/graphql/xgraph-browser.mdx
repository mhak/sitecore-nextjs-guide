...
# xGraph Browser
Sitecore's xGraph Browser is a tool that allows developers to explore and query Sitecore content using GraphQL. To get to it,
...
To execute a query, we must pass in an API key to query. You can find your API key under `/sitecore/system/Settings/Services/API Keys`. It is the item ID of the item under that path.
...
### 1. Simple item query
The first query we're going to do is build is a simple item query with a property of `datasource` as the field name. We will retrieve the `/sitecore/content/XmNextJs/Navigation/Header` item with `en` language
