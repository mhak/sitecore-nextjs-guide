---
title: Working in a docker environment
description: 
---
import { Callout } from 'nextra-theme-docs'

# Working in a docker environment

If you have setup your sitecore environment by following the [previous guide](/environment/installing-and-running-the-sitecore-environment), your Docker Desktop should look like something below.
You may noticed that `solr-init-1` and `mssql-init-1` is stopped. This is normal as those containers are only used to setup your Solr and SQL environment respectively.
![Docker Desktop - Sitecore XM Next.js](../../public/images/docker/docker-desktop-sitecore-xm-nextjs.png)

## docker compose stop
To stop your environment and free up resources on your computer, in Docker Desktop, click the **Stop** button under actions.
![Docker Desktop - stop](../../public/images/docker/docker-desktop-stop.png)

alternatively, back in VSCode, you can run
```
docker compose stop
```
![VSCode - docker compose stop](../../public/images/docker/vscode-docker-compose-stop.png)

If you try to visit your site (https://www.xmnextjs.localhost/), you will notice that it has stop working.

## docker compose start
To start your environment again, in Docker Desktop, click the **Start** button under actions.
![Docker Desktop - stop](../../public/images/docker/docker-desktop-start.png)

alternatively, back in VSCode, you can run
```
docker compose start
```

<Callout>
    Make sure you're under the `\XmNextJs\run\sitecore-xm1` because that is where our [docker-compose.yml](https://docs.docker.com/compose/compose-file/) file lives
</Callout>
![VSCode - docker compose start](../../public/images/docker/vscode-docker-compose-start.png)

## docker compose down
To delete your environment, in Docker Desktop, click the **Delete** button under actions.
![Docker Desktop - stop](../../public/images/docker/docker-desktop-delete.png)

alternatively, back in VSCode, you can run
```
docker compose down
```

![VSCode - docker compose down](../../public/images/docker/vscode-docker-compose-down.png)

## docker compose up
After deleting your environment, you can create your environment again by running the command below. Notice the `-d` flag as we are running under detached mode i.e. running it in the background.
<Callout>
    Make sure you're under the `\XmNextJs\run\sitecore-xm1` because that is where our [docker-compose.yml](https://docs.docker.com/compose/compose-file/) file lives
</Callout>
```
docker compose up -d
```

![VSCode - docker compose up](../../public/images/docker/vscode-docker-compose-up.png)

## Reference
Learn more about [docker compose](https://docs.docker.com/compose/reference/)